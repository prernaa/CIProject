...
function [Outs_Final,Outs_Sadjadi,Outs_New,t] = VAD_Drugman_prerna(wave,Fs,doPlot)
...
% Feature Extraction
[Feat,t] = VAD_Drugman_FeatureExtraction_prerna(wave,Fs);
...
Outs_Final=(Outs_Sadjadi.*Outs_New).^(1/2);
if doPlot

    fsp = Fs;
    durS = length(wave);
    durT = double(length(wave))/fsp;
    dt = 1/fsp;
    tAxis = dt:dt:durT;
